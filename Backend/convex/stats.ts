...
export const getAdminStats = query({
    ...
    handler: async (ctx, args) => {
        ...
        let outgoingReferrals: any[] = [];

        // If Super Admin, they have no hospitalId but we still want to show them stats
        if (hospitalId) {
...
export const getSuperAdminStats = query({
    ...
    handler: async (ctx) => {
        ...
        if (user.role !== "super_admin") {
            throw new Error(`Only super_admins can fetch system-wide statistics. Your current role is: ${user.role} (${user.email})`);
        }
